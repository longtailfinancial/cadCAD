...
from cadCAD.configuration.utils import exo_update_per_ts, configs_as_objs, filter_jobs
from cadCAD.utils import key_filter, re_assign_modules, re_encode


class Configuration(): #jobs
...
class Experiment(object):
    def __init__(self):
        ...
        self.filter_jobs = curry(filter_jobs)(self)
    ...
    def serialize_jobs(self, new_module_name=None):
        if new_module_name is not None:
            exp = re_assign_modules(self, new_module_name)
            _configs = exp.configs
        else:
            _configs = self.configs

        for model_id in self.model_ids:
            job_list = [re_encode(dill.dumps(job)) for job in self.model_job_map[model_id]]
            self.model_ser_job_map[model_id] = job_list
            self.model_ser_job_counts[model_id] = len(job_list)

        self.ser_configs = list(map(lambda x: re_encode(dill.dumps(x)), _configs))
        return self.model_ser_job_map

    # def send(self, endpoints):
    #     if type(endpoints) is list:
    #         endpoint_map = list(zip(self.ser_configs, endpoints))
    #     elif type(endpoints) is dict:
    #         endpoint_map = endpoints.items()
    #     print()
    #     for ser_config, endpoint in endpoint_map:
